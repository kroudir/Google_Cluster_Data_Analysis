...
for i in range(10):
    ...
    dir = Path("./data/task_events/")
    df = (pd.read_csv(f,compression='gzip',names=["timestamp","missing_info","job_id","task_index_job","machine_id","event_type",
                               "username","scheduling_class","priority","cpu_request","memory_request",
                               "disk_request", "machine_restriction"]) for f in dir.glob("*.csv.gz"))
    
    df_task_event = pd.concat(df, )
    
    dir = Path("./data/task_usage/")
    df = (pd.read_csv(f,compression='gzip',names=["start_time","end_time","job_id","task_index_job","machine_id",
                                ...
                                "agg_type","cpu_usage"]) for f in dir.glob("*.csv.gz"))
