...
for i in range(10):
    ...
    dir = Path("hdfs:///task_events/")
    hdfs_parts = [hd.read(path) for path in dir.glob("*.csv.gz")]
    df = (pd.read_csv(part,compression='gzip',names=["timestamp","missing_info","job_id","task_index_job","machine_id","event_type",
                               "username","scheduling_class","priority","cpu_request","memory_request",
                               "disk_request", "machine_restriction"]) for part in hdfs_parts)
    
    df_task_event = pd.concat(df, )
    

    dir = Path("hdfs:///task_usage/")
    hdfs_parts = [hd.read(path) for path in dir.glob("*.csv.gz")]
    df = (pd.read_csv(part,compression='gzip',names=["start_time","end_time","job_id","task_index_job","machine_id",
                                ...
                                "agg_type","cpu_usage"]) for part in hdfs_parts)
    df_task_usage = pd.concat(df, )
    ...
    joined_df = pd.merge(df_task_event, df_task_usage, on=['job_id','task_index_job'])
    ...
    tasks_asking_for_less_resources_pourcentage = less_ressources.count()[0] / joined_df.count()[0] * 100
...
pandas_times = [mean(times_read_data),
               mean(times_select),
               mean(times_join),
